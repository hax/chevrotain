...
class ECMAScript5Parser extends Parser {
    constructor(input) {
        ...
        $.RULE("ConditionalExpressionNoIn", () => {
            ...
            $.OPTION(() => {
                $.CONSUME(t.Question)
                $.SUBRULE($.AssignmentExpression) // TODO: why does spec not say "NoIn" here?
                $.CONSUME(t.Colon)
        ...
        $.RULE("Statement", () => {
            $.OR(
                [
                    ...
                    { ALT: () => $.SUBRULE($.EmptyStatement) },
                    {
                        ALT: () => {
                            ...
                            $.OPTION({
                                GATE: () => {
                                    // TODO: replace instantOf
                                    return (
                                        tokenMatcher(this.LA(1), t.Semicolon) &&
                                        isOnlyIdentifierStatement
                                    )
                                },
                                DEF: () => {
                                    $.CONSUME(t.Colon)
                ...
                // TODO: handle this flag? how is it implemented in modern versions of Chevrotain?
        ...
        $.RULE("SourceElements", () => {
            $.MANY(() => {
                ...
                $.OR(
                    ...
                    // TODO: what is the modern equivalent of recog.IGNORE_AMBIGUITIES?
                    // recog.IGNORE_AMBIGUITIES
                )
            })
        })
    ...
    CONSUME(tokClass, trySemiColonInsertion = false) {
    ...
    CONSUME2(tokClass, trySemiColonInsertion = false) {
        if (trySemiColonInsertion && this.canAndShouldDoSemiColonInsertion()) {
            const insertedSemiColon = {
                tokenType: t.Semicolon.tokenType,
                image: ";",
